...
const {User, Category, Brand, Unit, Supplier, Product, Account, Coustomer, Cart, Coupon, Order} = require('../model/Schema');
...
router.get('/pos', isAuth , async (req, res)=>{
    try { 
        ...
        const catList = await Category.aggregate([{$match: {status: {$eq:'active'}}},{$project:{_id:1, catName:1}}]);
        var activecat = catList.map((data)=>{
            return data.catName
        })
        
        var match = {category : {$in :activecat}}
        const product = await Product.aggregate([{$match: match},{$project:{_id:1, Name:1, proCode:1,  sellingPrice:1, productImage:1 }}]);
       ...
       if(cart){
            ...
            if(cart.coustomerId != ''){
                var coustomerName = await Coustomer.aggregate([{$match: {_id : {$eq: ObjectId(cart.coustomerId) }}}, {$project: {_id:1, cousName: 1}}]);
...
router.get('/srchcat/:id', isAuth, async (req, res)=>{
    try { 
        ...
        const catList = await Category.aggregate([{$match: {status: {$eq:'active'}}},{$project:{_id:1, catName:1}}]);
            ...
            const product = await Product.aggregate([{$match: {category : {$eq :catname}} },{$project:{_id:1, Name:1, proCode:1,  sellingPrice:1, productImage:1 }}]);
            var costomer = await Coustomer.find({});
...
router.post('/addCart',  async (req, res)=>{
    try { 
       ...
       if(product.discountType == "percent") {
        discount = ((parseInt(product.sellingPrice)*parseInt(product.discount))/100).toFixed(2)
        }
       var tax = ((parseInt(product.sellingPrice)*parseInt(product.tax))/100).toFixed(2)
       var total =( parseInt(product.sellingPrice) - parseInt(discount) +parseInt(tax)).toFixed(2)
        ...
        if(cartdata){
            ...
            cartdata.SubTotal = parseInt(cartdata.SubTotal) + parseInt(product.sellingPrice),
            cartdata.Productdiscount = parseInt(cartdata.Productdiscount) + parseInt(discount)
...
router.post('/userId',  async (req, res)=>{
    try {
        ...
        const coustomerName = await Coustomer.aggregate([{$match: {_id : {$eq: ObjectId(req.body.userId) }}}, {$project: {_id:0, cousName: 1}}]);
...
router.post('/quntity',  async (req, res)=>{
    ...
    try {var quntity = req.body.quntity
        const cart = await Cart.findOne({});
        const proQuntity = await Product.aggregate([{$match: {Name : {$eq: req.body.proName}}},{$project: {_id:0, quantity:1}}])
        ...
        cart.item.forEach(function(pro){
            if(pro._id.toString() == req.body.id){
                ...
                pro.total = ((pro.productPrice + pro.tax - pro.discount)* quntity).toFixed(2)
                cart.SubTotal =( cart.SubTotal + (pro.productPrice *(quntity - oldcount))).toFixed(2);
                cart.Productdiscount = (cart.Productdiscount + (pro.discount *(quntity - oldcount))).toFixed(2);
                cart.Tax = (cart.Tax + (pro.tax *(quntity - oldcount))).toFixed(2);
                cart.Amount = (cart.SubTotal + cart.Tax - cart.Productdiscount - cart.Coupondiscount).toFixed(2);
            }
        })
    ...
    try {
            ...
            cart.item.forEach(function(pro, index){
                ...
                if(pro._id.toString() == req.params.id){
                    ...
                    cart.SubTotal = (cart.SubTotal - (pro.productPrice * pro.productCount)).toFixed(2);
                    cart.Productdiscount = (cart.Productdiscount - (pro.discount * pro.productCount)).toFixed(2);
...
router.post('/coupon', async (req, res)=>{
    try {
        ...
        var cart = await Cart.findOne({});
        var coupon = await Coupon.aggregate([{$match:  {$and: [{Code: {$eq:coupon_code}}, {status: {$eq: "active"}} ,{"startDate":{"$lte":new Date()}}, {"endDate":{"$gte":new Date()}}]}}]);
        ...
        var discount = (coupon[0].disAmount).toFixed(2)
...
router.get('/payment', async (req, res)=>{
    try {   
        var cart = await Cart.findOne({});
        const account = await Account.aggregate([{$match : {$and: [ {"accTitel": {$ne: 'Receivable'}},{"accTitel": {$ne: 'Payable'} }]  }},
    ...
    try {   
       ...
       //*******if coupon applied then save coustomer id to this coupon user detail*************/
        ...
        var Paymethod = {
          ...
          if( req.body.transref){Paymethod.tranRef = req.body.transref};
          if( req.body.colleCash){Paymethod.amount = req.body.colleCash};
          if( req.body.returnCash){Paymethod.change = req.body.returnCash};
                            ...
                            //******* if order amount is gretar then wallet balance then debet from payabale account as availabale*/
                                        ...
                                        if(coustomer.Balance > 0){
                                            ...
                                            payableAccoount.transaction =   payableAccoount.transaction.concat({
                                                ...
                                                credit: 0,
                                                balance:( parseInt(payTotal) - parseInt(coustomer.Balance)).toFixed(2),
                                                date: new Date()
                                            })
                                            await payableAccoount.save();
                                            ...
                                            var remainAmount = parseInt(cart.Amount) - parseInt(coustomer.Balance)
