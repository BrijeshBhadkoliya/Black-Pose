...
const {Unit, User} = require('../model/Schema');



//add brand get router
router.get('/unit', isAuth, async (req,res)=>{
    try { const unitList = await Unit.find({});
...
router.post('/unit-update', isAuth, async (req, res)=>{
    try {
        ...
        if(unitName){
            req.flash('errors', `${req.body.unit} alredy added please choose onother`)
            return res.redirect('/unit/unit') 
        } 

        var unitList = await Unit.findByIdAndUpdate(req.body.editid,{unit:req.body.unit});
